...
uint256 hashGenesisBlock("0x000007ae1f61a0292afb033c09fa60ff716cd3419340a4f91d6a38676d9043d1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // QUEDOS: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "QUEDOS Signed Message:\n";
...
{
    // QUEDOS: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // QUEDOS: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "QUEDOS GENESIS";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1443463495;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1342607;
        ...
        assert(block.hashMerkleRoot == uint256("0xb4ace276bb3d100fbf9b09227ebbf1d043a3c90d8832e0fdbb3a1cb1c60f1556"));
...
unsigned char pchMessageStart[4] = { 0xee, 0xe3, 0xbe, 0xfe }; // QUEDOS: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QUEDOSMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QUEDOSMiner : ProcessBlock, block not accepted");
    }
